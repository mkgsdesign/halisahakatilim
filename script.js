...
function getRating(name) {
    ...
    return playerProfiles[name]
        ? playerProfiles[name].rating
        : DEFAULT_RATING;
}


/* =========================================================
   OYUNCU SIRA NUMARASI
========================================================= */
...
function setupVideos() {
    ...
    if (notComingVideo) {
        ...
        notComingVideo.addEventListener(
            "ended",
            () => hideVideo("notComing")
        );
    }
...
function updatePlayers(data) {
     ...
     * ÇOK ÖNEMLİ:
     *
     * İSİM ÜZERİNDEN EŞLEŞTİRME YAPMIYORUZ.
     *
     * Sheet sırası kullanılıyor.
     *
     * data.players[0]
     * -> players[0]
     *
...
function updateAttendanceLists() {
    ...
    renderTags(
        "comingList",
        getComingPlayers(),
        "Henüz kimse katılım bildirmedi."
    );
...
function setPayment() {
    ...
    if (
        !confirm(
            `💳 ${MATCH_FEE} TL ödeme yaptığınızı onaylıyor musunuz?`
        )
    ) {
    ...
    sendPost(
        ...
        error => {
            ...
            if (error) {
                ...
                alert(
                    "❌ Ödeme kaydedilemedi, tekrar dene."
                );
            }
        }
    );
...
function clearSelectedPlayer() {
    ...
    if (
        !confirm(
            `${selectedPlayer} için durumu temizlemek istediğine emin misin?`
        )
    ) {
...
function copyIban() {
    ...
    const iban =
        ibanText.dataset.iban ||
        ibanText.textContent.trim();
                    ...
                    return `
                        ...
                        <div class="admin-player-row">
                            ...
                            <div class="admin-rating-controls">
                                ...
                                <span class="admin-player-status ${statusClass}">
...
function handleAdminRatingChange(
    event
) {
    ...
    /*
     * Burada da isim yerine
     * sıra numarası gönderiyoruz.
     */
...
function generateTeams() {
    ...
    if (
        ...
        alert(
            "Takım oluşturmak için en az 2 kişi gelmeli."
        );
    ...
    if (
        ...
        container.innerHTML = `
            ...
            <div class="history-item">
                ...
                <p>
                    Bu haftanın maç geçmişi henüz oluşturulmadı.
                </p>
...
function deleteHistory() {
    ...
    if (
        !confirm(
            "Maç geçmişini temizlemek istediğine emin misin?"
        )
    ) {
...
function clearWeek() {
    ...
    if (
        !askAdminPassword(
            "Yeni haftayı başlatmak için admin şifresini gir:"
        )
    ) {
    ...
    if (
        !confirm(
            "Bu haftanın tüm katılım ve ödeme durumlarını temizlemek istediğine emin misin?\n\n(Oyuncu puanları silinmez.)"
        )
    ) {
    ...
    sendPost(
        ...
        error => {
            ...
            if (error) {
                ...
                alert(
                    "❌ Yeni hafta başlatılamadı, tekrar dene."
                );
            }
        }
    );
